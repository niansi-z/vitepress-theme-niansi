...
import { pathToFile } from '../../support/utils'
...
const defaultTranslations: LocalSearchTranslations = {
  ...
  modal: {
    ...
    footer: {
      resultsTextAfter: 'Found',
      resultsTextEnd: 'results',
      ...
      <div class="shell">
        <form class="search-bar" @pointerup="onSearchBarClick($event)" @submit.prevent="">
          <label :title="translate('button.buttonText')" id="localsearch-label" for="localsearch-input">
            <span aria-hidden="true" class="nsi-search search-icon local-search-icon" />
          </label>
          <div class="search-actions before">
            <button class="back-button" :title="translate('modal.backButtonTitle')" @click="$emit('close')">
              <span class="nsi-arrow-left local-search-icon" />
            </button>
          </div>
          <input
            ref="searchInput"
          ...
          <div class="search-actions">
            ...
            >
              <span class="nsi-layout-list local-search-icon" />
            </button>
            ...
            <button
              ...
              @click="resetSearch"
            >
              <span class="nsi-delete local-search-icon" />
            </button>
          </div>
        </form>
              ...
              <div>
                <div class="titles">
                  ...
                  <span v-for="(t, index) in p.titles" :key="index" class="title">
                    ...
                    <span class="nsi-chevron-right local-search-icon" />
                  </span>
                ...
                <div v-if="showDetailedList" class="excerpt-wrapper">
                  <div v-if="p.text" class="excerpt" inert>
                    <div class="ns-doc" v-html="p.text" />
                  </div>
...
.shell {
  ...
  width: min(100vw - 3.75rem, 56rem);
  height: min-content;
  max-height: min(100vh - 8rem, 56rem);
  border-radius: 10px;
  overflow: hidden;
}
...
.search-keyboard-shortcuts .search-results-count {
  margin-right: auto;
}

@media (max-width: 849px) {
  .search-keyboard-shortcuts span:not(.search-results-count) {
    display: none;
  }
}
...
.result {
  ...
  transition: none;
  line-height: 1rem;
  border: solid 2px var(--ns-border-color);
  outline: none;
}
...
.excerpt-gradient-bottom {
  ...
  width: 100%;
  height: 0.5rem;
  background: linear-gradient(transparent, var(--ns-bg));
  z-index: 1000;
}
...
.excerpt-gradient-top {
  ...
  width: 100%;
  height: 0.5rem;
  background: linear-gradient(var(--ns-bg), transparent);
  z-index: 1000;
}

.result.selected .titles,
.result.selected .title-icon {
  color: var(--ns-link-color) !important;
}
