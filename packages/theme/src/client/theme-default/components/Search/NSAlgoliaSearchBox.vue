...
const props = defineProps<{
  algoliaOptions: NiansiTheme.AlgoliaSearchOptions
  openRequest?: {
...
async function update(options: NiansiTheme.AlgoliaSearchOptions) {
  if (!inBrowser) return
  await nextTick()
...
async function initialize(userOptions: NiansiTheme.AlgoliaSearchOptions) {
  ...
  if (useSidePanel && askAi?.sidePanel) {
    ...
    sidepanelInstance = sidepanel({
      ...
      container: '#ns-docsearch-sidepanel',
