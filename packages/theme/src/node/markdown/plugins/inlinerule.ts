...
import type { InlineRuleOptions } from '../../../../types'

const UNESCAPED_SPACES_OR_NEW_LINES_RE = /(^|[^\\])(\\\\)*\s/u

const UNESCAPE_RE: RegExp = /\\([ \\!"#$%&'()*+,./:;<=>?@[\]^_`{|}~-])/gu
...
const createLinearRule = (config: LinearRuleConfig): RuleInline => {
  ...
  return (state, silent): boolean => {
    ...
    if (state.src.charCodeAt(start) !== markerCode || silent || start + 2 * markerLength >= max) return false
...
const createRulerHandler =
  (postProcess: (state: StateInline, delimiters: Delimiter[]) => void): ((state: StateInline) => boolean) =>
  (state): boolean => {
...
export const inlineRule = (md: MarkdownItAsync, options: InlineRuleOptions) => {
  if (
    !options ||
  ...
  if (marker.length !== 1) throw new Error("Invalid marker for inlineRule plugin: 'marker' must be a single character.")
  ...
  if (nested) {
    ...
    const rulerHandler = createRulerHandler(postProcess)

    if (isBefore) {
      md.inline.ruler.before('emphasis', ruleName, tokenize)
      md.inline.ruler2.before('emphasis', ruleName, rulerHandler)
    } else {
      md.inline.ruler.after('emphasis', ruleName, tokenize)
      md.inline.ruler2.after('emphasis', ruleName, rulerHandler)
    }
  } else {
